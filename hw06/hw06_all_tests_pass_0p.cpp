...
int main() {
    ...
    std::vector<size_t> good(M, 0);
    std::vector<size_t> numbersToM(M , 0);
    ...
    for(size_t i = 2; i < M; i++) {
        if(numbersToM[i] != 1) {
            size_t prime = numbersToM[i];
            for(size_t j = prime; j < M; j=j+prime) {
                if((numbersToM[j] % prime) == 0) {
                    numbersToM[j] = numbersToM[j] / prime;
